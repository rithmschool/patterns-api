...
describe('Asset model', function() {
  ...
  describe('Pre-save hook', function() {
    ...
    it("should update its type's array of assets", function(done) {
      ...
      db.Type
        ...
        .then(function(companyNew) {
          expect(companyNew.assets.length).to.equal(
            companyOld.assets.length + 1
          );
    ...
    it("should update its parent asset's array of assets, if it has a parent", function(
      done
    ) {
      let googleOld = null;
      let brandId = null;
      db.Asset
        ...
        .then(function(googleNew) {
          ...
          expect(googleNew.assets.map(a => a.name)).to.include(
            'Google New Brand'
          );
          done();
        })
  ...
  describe('Pre-remove hook', function() {
    ...
    it("should be removed from its parent's array of assets, if it has a parent", function(
      done
    ) {
      db.Asset
        .findOne({ name: 'Google' })
    ...
    it('should be removed from all stages containing it', function(done) {
      db.User
        ...
        .then(function(bob) {
          ...
          expect(
            allStages.find(s => s.name === "Bob's Ideas").assets.length
          ).to.equal(2);
          expect(
            allStages.find(s => s.name === "Bob's Other Ideas").assets.length
          ).to.equal(1);
          expect(
            allStages.find(s => s.name === "Bob's Other Research").assets.length
          ).to.equal(1);
          return db.Asset.findOne({ name: 'Google' });
        })
        .then(function(google) {
